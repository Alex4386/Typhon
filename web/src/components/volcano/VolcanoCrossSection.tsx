...
function computeGeo(vent: VentDetail): Geo {
  ...
  const drawableHalfW = (SVG_W - MARGIN_X * 2) / 2;
  ...
  return {
    ...
    summitPx, craterPxR, calderaPxR, flowPxR, bombPxR,
    isCaldera, craterRadius, calderaRadius,
    flowLength, rawFlowLength, bombMax,
...
function buildProfilePath(geo: Geo): string {
  const {
    summitPx, craterPxR, calderaPxR, flowPxR, isCaldera, craterDepthPx,
  } = geo;
  ...
  // Start at left base
  parts.push(`M ${CENTER_X - flowPxR} ${GROUND_Y}`);
  ...
  const leftRimX = CENTER_X - craterPxR;
  const slopeMidX_L = CENTER_X - flowPxR * SLOPE_CP_X;
  ...
  if (isCaldera && calderaPxR > craterPxR) {
    ...
    const outerRimL = CENTER_X - calderaPxR;
    const outerRimR = CENTER_X + calderaPxR;
    ...
    parts.push(`Q ${outerRimL + calderaPxR * CALDERA_WALL_CP_X} ${calderaFloor - craterDepthPx * CALDERA_WALL_CP_Y} ${CENTER_X - calderaPxR * CALDERA_FLOOR_W} ${calderaFloor}`);
    // Caldera floor
    parts.push(`L ${CENTER_X + calderaPxR * CALDERA_FLOOR_W} ${calderaFloor}`);
    ...
    parts.push(`L ${CENTER_X + craterPxR} ${rimY}`);
  } else {
    // Normal crater bowl: left rim -> dip down into crater -> right rim
    // Smooth parabolic bowl using a quadratic bezier
    parts.push(`Q ${CENTER_X} ${craterFloorY + craterDepthPx * CRATER_BOWL_CP_Y} ${CENTER_X + craterPxR} ${rimY}`);
  }

  // Right slope: mirror of left (concave curve from rim down to base)
  const rightRimX = CENTER_X + craterPxR;
  const slopeMidX_R = CENTER_X + flowPxR * SLOPE_CP_X;
  parts.push(`Q ${slopeMidX_R} ${slopeMidY} ${CENTER_X + flowPxR} ${GROUND_Y}`);
...
export function VolcanoCrossSectionRaw({ vent }: Props) {
  const geo = useMemo(() => computeGeo(vent), [vent]);
    ...
    >
      <defs>
        ...
        <clipPath id={`crater-clip-${uid}`}>
          <path
            d={`M 0 0 H ${SVG_W} V ${SVG_H} H 0 Z ${profilePath}`}
            clipRule="evenodd"
          />
        </clipPath>
        ...
        {geo.seaLevelPxY != null && (
          <clipPath id={`sea-clip-${uid}`}>
            <rect x="0" y={geo.seaLevelPxY} width={SVG_W} height={GROUND_Y - geo.seaLevelPxY} />
          </clipPath>
        )}
      </defs>
      ...
      {/* ── 1. Background ──────────────────────────────────────────── */}
      {/* Sky */}
      <rect x="0" y="0" width={SVG_W} height={GROUND_Y} fill="#0d1117" opacity="0.4" />

      {/* Ground */}
      <rect x="0" y={GROUND_Y} width={SVG_W} height={SVG_H - GROUND_Y} fill={`url(#ground-${uid})`} />
      {/* Ground surface line */}
      <line x1="0" y1={GROUND_Y} x2={SVG_W} y2={GROUND_Y} stroke="#4a3a28" strokeWidth="1" />

      {/* ── 2. Sea level water body (behind mountain) ─────────────── */}
      {geo.seaLevelPxY != null && (
        <rect
          x="0" y={geo.seaLevelPxY}
          width={SVG_W} height={GROUND_Y - geo.seaLevelPxY}
      ...
      {geo.flowPxR > 0 && (
        <g>
          <ellipse
            cx={CENTER_X} cy={GROUND_Y}
            rx={geo.flowPxR} ry={6}
            fill={`url(#lava-r-${uid})`}
          />
          <rect
            x={CENTER_X - geo.flowPxR}
            y={GROUND_Y - 2}
            width={geo.flowPxR * 2}
            height={4}
            rx={2}
      ...
      {/* ── 7. Bomb range arc (reference overlay) ─────────────────── */}
      {geo.bombPxR > 0 && (
        <g>
          <path
            d={`M ${CENTER_X - geo.bombPxR} ${GROUND_Y} A ${geo.bombPxR} ${geo.bombPxR} 0 0 1 ${CENTER_X + geo.bombPxR} ${GROUND_Y}`}
            fill="none" stroke="#ff6b35" strokeWidth="1" strokeDasharray="4 3" opacity="0.4"
          />
          <text
            x={CENTER_X + geo.bombPxR + 4} y={GROUND_Y - geo.bombPxR * 0.5}
            fill="#ff6b35" fontSize="9" fontFamily="monospace" opacity="0.5"
          >
            Bomb {geo.bombMax.toFixed(2)}m
          </text>
        </g>
      )}
      ...
      {geo.flowPxR > 0 && (() => {
        ...
        const craterEdgePx = CENTER_X + geo.craterPxR;
        ...
        return (
          <g>
            ...
            <g>
              ...
              <line
                x1={CENTER_X - geo.flowPxR} y1={GROUND_Y - 5}
                x2={CENTER_X - geo.flowPxR} y2={GROUND_Y + 5}
              ...
              {/* Center tick */}
              <line
                x1={CENTER_X} y1={GROUND_Y + 30}
                x2={CENTER_X} y2={GROUND_Y + 38}
                stroke="#888" strokeWidth="0.5" opacity="0.4"
              />
              {/* Dimension line: center → right basin edge */}
              <line
                x1={CENTER_X} y1={GROUND_Y + 34}
                x2={CENTER_X + geo.flowPxR} y2={GROUND_Y + 34}
                stroke="#888" strokeWidth="0.5" opacity="0.35"
              />
              {/* Right tick */}
              <line
                x1={CENTER_X + geo.flowPxR} y1={GROUND_Y + 30}
                x2={CENTER_X + geo.flowPxR} y2={GROUND_Y + 38}
                stroke="#888" strokeWidth="0.5" opacity="0.4"
              />
              {/* Label */}
              <text
                x={CENTER_X + geo.flowPxR / 2} y={GROUND_Y + 48}
                fill="#888" fontSize="9" fontFamily="monospace" textAnchor="middle" opacity="0.5"
              >
                Basin r={geo.flowLength.toFixed(0)}m
              </text>
            </g>
          </g>
        );
      ...
      {/* ── 10. Crater glow (ERUPTION_IMMINENT+), clipped to mountain */}
      {scaleFactor >= 0.7 && (
        <g clipPath={`url(#crater-clip-${uid})`}>
          <ellipse
            cx={CENTER_X}
            cy={geo.summitPx + geo.craterDepthPx * 0.6}
            rx={geo.craterPxR * 0.6}
            ry={Math.max(geo.craterDepthPx * 0.25, 2)}
            fill={lavaGlow}
            opacity={0.15 + (intensity - 0.7) * 1.5}
          >
            <animate
              attributeName="opacity"
              values={`${0.1 + (intensity - 0.7)};${0.3 + (intensity - 0.7) * 1.5};${0.1 + (intensity - 0.7)}`}
...
function Labels({ geo, vent }: { geo: Geo; vent: VentDetail }) {
  return (
    <g>
      {/* Summit Y */}
      <line
        x1={CENTER_X + geo.craterPxR + 8} y1={geo.summitPx}
        x2={CENTER_X + geo.craterPxR + 30} y2={geo.summitPx}
        stroke="#999" strokeWidth="0.5"
      />
      <text
        x={CENTER_X + geo.craterPxR + 33} y={geo.summitPx + 3}
      ...
      <line
        x1={CENTER_X - geo.craterPxR} y1={geo.summitPx - 8}
        x2={CENTER_X + geo.craterPxR} y2={geo.summitPx - 8}
        stroke="#aaa" strokeWidth="0.5"
      />
      {/* Ticks */}
      <line
        x1={CENTER_X - geo.craterPxR} y1={geo.summitPx - 12}
        x2={CENTER_X - geo.craterPxR} y2={geo.summitPx - 4}
        stroke="#aaa" strokeWidth="0.5"
      />
      <line
        x1={CENTER_X + geo.craterPxR} y1={geo.summitPx - 12}
...
// Hawaiian: gentle lava fountain arcs
function HawaiianEffect({ geo, intensity, lavaColor }: EffectProps) {
  ...
  const particles = useMemo(() =>
    Array.from({ length: n }, (_, i) => {
      ...
      return { spread, h, delay: (i * 0.25) % 3, dur: 1.0 + (i % 4) * 0.3 };
    }),
  [n, geo.craterPxR, intensity]);
  ...
  return (
    <g>
      {/* Glow at crater */}
      <ellipse cx={CENTER_X} cy={craterY} rx={geo.craterPxR * 0.5} ry={6} fill={lavaColor} opacity={0.35 * intensity}>
        <animate attributeName="opacity" values={`${0.25 * intensity};${0.5 * intensity};${0.25 * intensity}`} dur="1.5s" repeatCount="indefinite" />
      </ellipse>
      {/* Fountain particles: rise and fall in parabolic arcs */}
      {particles.map((p, i) => (
        <circle key={i} r={1.5 + (i % 2)} fill={i % 3 === 0 ? '#ffcc00' : lavaColor} opacity="0">
          <animate attributeName="cx" values={`${CENTER_X};${CENTER_X + p.spread * 0.5};${CENTER_X + p.spread}`} dur={`${p.dur}s`} begin={`${p.delay}s`} repeatCount="indefinite" />
          <animate attributeName="cy" values={`${craterY};${craterY - p.h};${craterY + 10}`} dur={`${p.dur}s`} begin={`${p.delay}s`} repeatCount="indefinite" />
...
// Strombolian: discrete bomb trajectories arcing outward
function StrombolianEffect({ geo, intensity, lavaColor }: EffectProps) {
  const n = Math.ceil(intensity * 6);
  const bombs = useMemo(() =>
    Array.from({ length: n }, (_, i) => {
      const side = i % 2 === 0 ? 1 : -1;
      const landDist = (geo.craterPxR * 2 + (i + 1) * geo.bombPxR * 0.15) * side;
      const peakH = 40 + (i % 3) * 25 * intensity;
      const sx = CENTER_X;
      const sy = geo.summitPx + geo.craterDepthPx * 0.2;
      const ex = CENTER_X + landDist;
      const ey = GROUND_Y;
      const cpx = (sx + ex) / 2;
      const cpy = sy - peakH;
  ...
  return (
    <g>
      {/* Flash at vent */}
      <circle cx={CENTER_X} cy={geo.summitPx + geo.craterDepthPx * 0.2} r={geo.craterPxR * 0.4} fill={lavaColor} opacity="0">
...
// Vulcanian: explosive burst + rising ash cloud
function VulcanianEffect({ geo, intensity, lavaColor }: EffectProps) {
  ...
  return (
    <g>
      {/* Burst flash */}
      <circle cx={CENTER_X} cy={geo.summitPx} r={burstR} fill={lavaColor} opacity="0">
      ...
      {/* Rising ash cloud */}
      <ellipse cx={CENTER_X} cy={geo.summitPx} rx={cloudMaxR * 0.3} ry={cloudMaxR * 0.2} fill="#4a4a4a" opacity="0">
      ...
      {/* Vent glow */}
      <ellipse cx={CENTER_X} cy={geo.summitPx + geo.craterDepthPx * 0.3} rx={geo.craterPxR * 0.7} ry={4} fill={lavaColor}>
...
function PeleanEffect({ geo, intensity, lavaColor }: EffectProps) {
  ...
  return (
    <g>
      {/* Dome */}
      <ellipse cx={CENTER_X} cy={craterY} rx={domeR} ry={domeR * 0.5} fill={lavaColor} opacity="0.6">
      ...
      <path
        d={`M ${CENTER_X - geo.craterPxR * 0.5} ${craterY + 8}
            Q ${CENTER_X - geo.flowPxR * 0.3} ${GROUND_Y - (GROUND_Y - craterY) * 0.3}
              ${CENTER_X - flowDist} ${GROUND_Y}
            L ${CENTER_X - flowDist * 0.5} ${GROUND_Y} Z`}
        fill="#666" opacity="0"
      >
  ...
  return (
    <g>
      ...
      >
        ...
        <animate attributeName="x" values={`${CENTER_X - colW * 0.425};${CENTER_X - colW * 0.575};${CENTER_X - colW * 0.425}`} dur="1.8s" repeatCount="indefinite" />
      </rect>
      {/* Mushroom cap */}
      <ellipse cx={CENTER_X} cy={topY} rx={mushRx} ry={mushRy} fill="#3a3a3a" opacity="0">
      ...
      {/* Vent glow */}
      <ellipse cx={CENTER_X} cy={geo.summitPx + geo.craterDepthPx * 0.3} rx={geo.craterPxR * 0.8} ry={6} fill={lavaColor} opacity={0.4 * intensity}>
...
function LavaDomeEffect({ geo, intensity, lavaColor }: EffectProps) {
  ...
  return (
    <g>
      <ellipse cx={CENTER_X} cy={craterY} rx={domeRx} ry={domeRy} fill={lavaColor} opacity="0.55">
        <animate attributeName="ry" values={`${domeRy * 0.7};${domeRy};${domeRy * 0.7}`} dur="8s" repeatCount="indefinite" />
      </ellipse>
      {/* Hot crack glow on dome surface */}
      <ellipse cx={CENTER_X} cy={craterY - domeRy * 0.2} rx={domeRx * 0.3} ry={2} fill="#ff9960" opacity={0.2 * intensity}>
...
function GenericEruption({ geo, intensity, lavaColor }: EffectProps) {
  return (
    <ellipse cx={CENTER_X} cy={geo.summitPx + geo.craterDepthPx * 0.3} rx={geo.craterPxR * 0.6} ry={6} fill={lavaColor} opacity="0">
