...
public class VolcanoAsh {
    ...
    public void registerTask() {
        if (ashFallScheduleId < 0) {
            ashFallScheduleId = Bukkit.getScheduler().scheduleSyncRepeatingTask(TyphonPlugin.plugin, (Runnable) () -> {
    ...
    public void initialize() {
        this.vent.volcano.logger.log(VolcanoLogClass.ASH, "Intializing VolcanoAsh for vent "+vent.getName());
        this.registerTask();
    }

    public void shutdown() {
        this.vent.volcano.logger.log(VolcanoLogClass.ASH, "Shutting down VolcanoAsh for vent "+vent.getName());
        this.unregisterTask();
    }

    public void createAshPlume() {
        createAshPlume(TyphonUtils.getHighestLocation(this.vent.selectCoreBlock().getLocation()).add(0, 1, 0));
    }
    ...
    public void createAshPlume(Location loc) {
        ...
        if (style == VolcanoEruptStyle.HAWAIIAN) {
            ...
            double vulcanianMultiplier = (1.5 + (Math.random()));

            double multiplier = 1;

            if (style == VolcanoEruptStyle.VULCANIAN || style == VolcanoEruptStyle.PELEAN) {
                multiplier = vulcanianMultiplier;
                if (style == VolcanoEruptStyle.PELEAN) multiplier *= 1.5;
            }

            for (int i = 0; i < count * multiplier; i++) {
                VolcanoCircleOffsetXZ xz = VolcanoMath.getCenterFocusedCircleOffset(loc.getBlock(), this.vent.craterRadius);
    ...
    public void triggerAshFall() {
        triggerAshFall(TyphonUtils.getHighestLocation(this.vent.selectCoreBlock().getLocation()).add(0, 1, 0));
    }
